...
const categories = [
  {
    ...
    products: [
      ...
      { id: 5, name: "Pink Marble", image: pinkMarble }
    ]
  },
  {
    name: "Granite",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-gray-600 max-w-xl mx-auto">
            Explore our finest selection of natural stones
          </p>
        </div>
        ...
        <div className="space-y-12">
          {categories.map((category, index) => (
            <div key={category.name} className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
              {/* Category Header */}
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="text-right">
                  <span className="inline-block px-3 py-1 bg-[#0E5543]/10 text-[#0E5543] text-xs font-medium rounded-full">
                    Premium Quality
                  </span>
                </div>
              </div>
              ...
              <div className="sm:hidden relative mb-6">
                <div className="overflow-hidden">
                  <div className="flex transition-transform duration-300" style={{ transform: `translateX(-${(currentSlides[category.name] || 0) * 100}%)` }}>
                  ...
                  {category.products.map((_, idx) => (
                    <button
                      ...
                      className={`w-2 h-2 rounded-full transition-colors ${
                        idx === (currentSlides[category.name] || 0) ? 'bg-[#0E5543]' : 'bg-gray-300'
                      }`}
              ...
              <div className="hidden sm:grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-6">
                {category.products.map((product) => (
                  <div key={product.id} className="group cursor-pointer">
                    ...
                    <div className="mt-2">
                      <h3 className="text-sm font-medium text-gray-800 text-center group-hover:text-[#0E5543] transition-colors">
                        {product.name}
                      </h3>
                    </div>
                  </div>
              ...
              <div className="border-t border-gray-100 pt-4">
                <button 
                  className="w-full sm:w-auto text-white px-6 py-2.5 rounded-lg transition-colors duration-300 text-sm font-medium" 
